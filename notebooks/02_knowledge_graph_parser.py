...
class MathematicalPaperKnowledgeGraph(BaseModel):
    ...
    authors: List[Author]
    year: int
    doi_uri: Optional[str] = None
    arxiv_uri: Optional[str] = None
...
def create_example_knowledge_graph() -> MathematicalPaperKnowledgeGraph:
    ...
    kg = MathematicalPaperKnowledgeGraph(
        ...
        authors=[
            Author(name="Bettin", orcid_uri="https://orcid.org/0000-0002-3006-2586", wikidata_uri="http://www.wikidata.org/entity/Q110594348"),
            Author(name="Conrey", viaf_uri="http://viaf.org/viaf/22421453", wikidata_uri="http://www.wikidata.org/entity/Q1000627")
        ],
        year=2013,
        arxiv_uri="https://arxiv.org/abs/1111.0931"
    )
...
def export_to_jsonld(kg: MathematicalPaperKnowledgeGraph) -> Dict[str, Any]:
    ...
    return {
        ...
        "title": kg.title,
        "authors": to_dict(kg.authors),
        "year": kg.year,
        "doi_uri": kg.doi_uri,
        "arxiv_uri": kg.arxiv_uri,
        "objects": to_dict(kg.objects.values()),
        "proofs": to_dict(kg.proofs.values()),
        "dependencies": to_dict(kg.dependency_graph.edges) if kg.dependency_graph else [],
        "open_problems": to_dict(kg.open_problems.problems) if kg.open_problems else []
    }

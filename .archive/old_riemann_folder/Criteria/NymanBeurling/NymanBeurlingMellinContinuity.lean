...
/-!
# Field 1: Mellin evaluation continuity

This file proves the continuity estimate for the Mellin functional on the
finite Báez--Duarte approximants.  The Mellin kernel is the actual Mathlib
kernel `x ^ (s - 1)`; on `(0,1]` its square norm is `x ^ (2*Re(s)-2)`, and on
`(1,∞)` the reciprocal tail gives the exponent `Re(s)-2`.
-/
...
private theorem measurable_bdApprox (N : ℕ) (coeffs : Fin N → ℝ) :
  ...
  unfold bdApprox
  exact Finset.measurable_sum _ (fun k hk => measurable_const.mul (measurable_rhoBD k.val))

private theorem measurable_mellinError (N : ℕ) (coeffs : Fin N → ℝ) :
    Measurable (mellinError N coeffs) := by
  unfold mellinError
  exact (measurable_chi01.sub (measurable_bdApprox N coeffs)).complex_ofReal
...
private theorem norm_bdApprox_le (N : ℕ) (coeffs : Fin N → ℝ) (x : ℝ) :
  ...
  unfold bdApprox
  calc
    ‖∑ x_1, coeffs x_1 * rhoBD x_1.val x‖ ≤
        ∑ x_1, ‖coeffs x_1 * rhoBD x_1.val x‖ := norm_sum_le _ _
    _ = ∑ x_1, ‖coeffs x_1‖ * ‖rhoBD x_1.val x‖ := by
      simp only [norm_mul]
    _ ≤ ∑ x_1, ‖coeffs x_1‖ := by
      apply Finset.sum_le_sum
      intro k hk
      exact mul_le_of_le_one_right (norm_nonneg _) (norm_rhoBD_le_one k.val x)

private theorem norm_mellinError_le (N : ℕ) (coeffs : Fin N → ℝ) (x : ℝ) :
    ‖mellinError N coeffs x‖ ≤ 1 + ∑ k : Fin N, ‖coeffs k‖ := by
  unfold mellinError
  rw [Complex.norm_real, Real.norm_eq_abs]
  calc
    |chi01 x - bdApprox N coeffs x| ≤ |chi01 x| + |bdApprox N coeffs x| :=
      abs_sub _ _
...
private theorem mellinError_aestronglyMeasurable_restrict
    (N : ℕ) (coeffs : Fin N → ℝ) (s : Set ℝ) :
    AEStronglyMeasurable (mellinError N coeffs) (volume.restrict s) := by
  exact (measurable_mellinError N coeffs).aestronglyMeasurable.restrict

private theorem mellinError_integrableOn_Ioc01
    (N : ℕ) (coeffs : Fin N → ℝ) :
    IntegrableOn (mellinError N coeffs) (Ioc (0 : ℝ) 1) := by
  apply Measure.integrableOn_of_bounded (μ := volume) (s := Ioc (0 : ℝ) 1)
    (M := 1 + ∑ k : Fin N, ‖coeffs k‖) measure_Ioc_lt_top.ne
    (measurable_mellinError N coeffs).aestronglyMeasurable
  filter_upwards [ae_restrict_mem measurableSet_Ioc] with x hx
  exact norm_mellinError_le N coeffs x

private def mellinKernel (s : ℂ) (x : ℝ) : ℂ :=
  (x : ℂ) ^ (s - 1)

private theorem mellinKernel_integrableOn_Ioc01 {s : ℂ} (hs : 0 < s.re) :
...
private theorem mellinKernel_norm_sq_integral {s : ℂ} (hs : 1 / 2 < s.re) :
    (∫ x in Ioc (0 : ℝ) 1, ‖mellinKernel s x‖ ^ 2) =
      1 / (2 * s.re - 1) := by
  have hexp : -1 < 2 * s.re - 2 := by linarith
  have hpow := integral_rpow (a := (0 : ℝ)) (b := 1) (r := 2 * s.re - 2)
    (Or.inl hexp)
  have heq :
      (∫ x in Ioc (0 : ℝ) 1, ‖mellinKernel s x‖ ^ 2) =
...
private theorem mellinKernel_memLp_Ioc01 {s : ℂ} (hs : 1 / 2 < s.re) :
    MemLp (mellinKernel s) 2 (volume.restrict (Ioc (0 : ℝ) 1)) := by
  apply (memLp_two_iff_integrable_sq_norm
    (mellinKernel_integrableOn_Ioc01 (by linarith)).aestronglyMeasurable).2
...
private theorem mellinError_memLp_Ioc01
    (N : ℕ) (coeffs : Fin N → ℝ) :
    MemLp (mellinError N coeffs) 2 (volume.restrict (Ioc (0 : ℝ) 1)) := by
  apply (memLp_two_iff_integrable_sq_norm
    (measurable_mellinError N coeffs).aestronglyMeasurable).2
  have hM : 0 ≤ 1 + ∑ k : Fin N, ‖coeffs k‖ := by positivity
  have hsq : IntegrableOn
      (fun x : ℝ => ‖mellinError N coeffs x‖ ^ 2) (Ioc (0 : ℝ) 1) := by
    apply Measure.integrableOn_of_bounded (μ := volume) (s := Ioc (0 : ℝ) 1)
      (M := (1 + ∑ k : Fin N, ‖coeffs k‖) ^ 2) measure_Ioc_lt_top.ne
      ((measurable_mellinError N coeffs).norm.pow_const 2).aestronglyMeasurable
    filter_upwards [ae_restrict_mem measurableSet_Ioc] with x hx
    have hnonneg : 0 ≤ ‖mellinError N coeffs x‖ ^ 2 := sq_nonneg _
    calc
      ‖‖mellinError N coeffs x‖ ^ 2‖ = ‖mellinError N coeffs x‖ ^ 2 :=
        Real.norm_of_nonneg hnonneg
      _ ≤ (1 + ∑ k : Fin N, ‖coeffs k‖) ^ 2 :=
        (sq_le_sq₀ (norm_nonneg _) hM).2 (norm_mellinError_le N coeffs x)
  exact hsq

private theorem realError_measurable (N : ℕ) (coeffs : Fin N → ℝ) :
    Measurable (realError N coeffs) := by
  exact measurable_chi01.sub (measurable_bdApprox N coeffs)

private theorem realError_sq_integrableOn_Ioc01
    (N : ℕ) (coeffs : Fin N → ℝ) :
    IntegrableOn (fun x => realError N coeffs x ^ 2) (Ioc (0 : ℝ) 1) := by
  refine Measure.integrableOn_of_bounded (μ := volume) (s := Ioc (0 : ℝ) 1)
    (M := (1 + ∑ k : Fin N, ‖coeffs k‖) ^ 2) measure_Ioc_lt_top.ne
...
private theorem realError_eq_tail_on_Ioi
  ...
  intro x hx
  exact NBMellinTools.chi_sub_bdApprox_eq_tail_of_one_lt N coeffs hx

private theorem realError_sq_integrableOn_Ioi
    (N : ℕ) (coeffs : Fin N → ℝ) :
    IntegrableOn (fun x => realError N coeffs x ^ 2) (Ioi (1 : ℝ)) := by
  have hpow : IntegrableOn (fun x : ℝ => x ^ (-2 : ℝ)) (Ioi (1 : ℝ)) :=
    integrableOn_Ioi_rpow_of_lt (by norm_num) one_pos
  have hconst : AEStronglyMeasurable (fun _ : ℝ => tailCoeff N coeffs ^ 2)
      (volume.restrict (Ioi (1 : ℝ))) := measurable_const.aestronglyMeasurable
  have hbound : ∀ᵐ x ∂(volume.restrict (Ioi (1 : ℝ))),
      ‖tailCoeff N coeffs ^ 2‖ ≤ |tailCoeff N coeffs ^ 2| := by
    filter_upwards with x
    rw [Real.norm_eq_abs]
  have hmodel : IntegrableOn
      (fun x : ℝ => tailCoeff N coeffs ^ 2 * x ^ (-2 : ℝ)) (Ioi (1 : ℝ)) := by
    have h := hpow.mul_bdd hconst hbound
    simpa only [mul_comm] using h
  apply hmodel.congr_fun _ measurableSet_Ioi
  intro x hx
  have hx0 : 0 < x := lt_trans zero_lt_one hx
  change tailCoeff N coeffs ^ 2 * x ^ (-2 : ℝ) = (realError N coeffs x) ^ 2
...
private theorem realError_sq_integrableOn_Ioi_zero
    (N : ℕ) (coeffs : Fin N → ℝ) :
    IntegrableOn (fun x => realError N coeffs x ^ 2) (Ioi (0 : ℝ)) := by
...
private theorem tailCoeff_sq_le_error
  ...
  have hpow : IntegrableOn (fun x : ℝ => x ^ (-2 : ℝ)) (Ioi (1 : ℝ)) :=
    integrableOn_Ioi_rpow_of_lt (by norm_num) one_pos
  have htail :
      (∫ x in Ioi (1 : ℝ), tailCoeff N coeffs ^ 2 * x ^ (-2 : ℝ)) =
...
private theorem mellin_integrand_integrableOn_Ioc01
    {s : ℂ} (hs : 0 < s.re) (N : ℕ) (coeffs : Fin N → ℝ) :
...
private theorem mellinError_local_sq_eq_realError
  ...
  intro x hx
  change ‖((realError N coeffs x : ℝ) : ℂ)‖ ^ 2 = realError N coeffs x ^ 2
  simp [Complex.norm_real, Real.norm_eq_abs, sq_abs]

theorem mellin_error_part_A_bound
    {s : ℂ} (hs : 1 / 2 < s.re) (N : ℕ) (coeffs : Fin N → ℝ) :
    ‖∫ x in Ioc (0 : ℝ) 1,
  ...
  have hcs := norm_setIntegral_mul_le_sqrt_mul_sqrt
    (mellinError N coeffs) (mellinKernel s) (Ioc (0 : ℝ) 1) volume
  ...
  have hrootk : 0 ≤ Real.sqrt
      (∫ x in Ioc (0 : ℝ) 1, ‖mellinKernel s x‖ ^ 2) := Real.sqrt_nonneg _
  calc
    ‖∫ x in Ioc (0 : ℝ) 1, mellinKernel s x * mellinError N coeffs x‖ =
        ‖∫ x in Ioc (0 : ℝ) 1, mellinError N coeffs x * mellinKernel s x‖ := by
          congr 1
          apply setIntegral_congr_fun measurableSet_Ioc
          intro x hx
          ring
    _ ≤ Real.sqrt
          (∫ x in Ioc (0 : ℝ) 1, ‖mellinError N coeffs x‖ ^ 2) *
          Real.sqrt (∫ x in Ioc (0 : ℝ) 1, ‖mellinKernel s x‖ ^ 2) := hcs
    _ ≤ Real.sqrt (BaezDuarteL2Error N coeffs) *
          Real.sqrt (∫ x in Ioc (0 : ℝ) 1, ‖mellinKernel s x‖ ^ 2) :=
...
private theorem mellinError_tail_model_eq
    {s : ℂ} (N : ℕ) (coeffs : Fin N → ℝ) {x : ℝ} (hx : 1 < x) :
    mellinKernel s x * mellinError N coeffs x =
...
private theorem mellin_tail_integrableOn_Ioi
    {s : ℂ} (hs : s.re < 1) (N : ℕ) (coeffs : Fin N → ℝ) :
    IntegrableOn (fun x => mellinKernel s x * mellinError N coeffs x)
      (Ioi (1 : ℝ)) := by
  ...
  have hmodel : IntegrableOn
      ...
      (Ioi (1 : ℝ)) := by
    have h := hcpow.mul_bdd (c := ‖( -(tailCoeff N coeffs) : ℂ)‖) hconst
...
theorem mellin_error_tail_part_B_bound
  ...
  have hint := mellin_tail_integrableOn_Ioi hs N coeffs
  have hpow : IntegrableOn
      (fun x : ℝ => Real.sqrt (BaezDuarteL2Error N coeffs) *
        x ^ (s.re - 2)) (Ioi (1 : ℝ)) := by
    have h := integrableOn_Ioi_rpow_of_lt (by linarith : s.re - 2 < -1) one_pos
    ...
    have hm := h.mul_bdd (c := |Real.sqrt (BaezDuarteL2Error N coeffs)|) hc hb
  ...
  have hbound : ∀ᵐ x ∂(volume.restrict (Ioi (1 : ℝ))),
    ...
    exact mul_le_mul_of_nonneg_right hcoeff (Real.rpow_nonneg hx0.le _)
  calc
    ‖∫ x in Ioi (1 : ℝ), mellinKernel s x * mellinError N coeffs x‖ ≤
        ∫ x in Ioi (1 : ℝ),
          Real.sqrt (BaezDuarteL2Error N coeffs) * x ^ (s.re - 2) :=
      norm_integral_le_of_norm_le hpow hbound
    _ = (1 / (1 - s.re)) * Real.sqrt (BaezDuarteL2Error N coeffs) := by
...
noncomputable def nyman_field_1 : MellinContinuityUnderL2Approx where
  statement := mellin_evaluation_continuous_on_critical_strip

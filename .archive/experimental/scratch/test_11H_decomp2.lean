...
theorem explicitCutoffMobiusEnergyKernel_eq_components (N : ℕ) :
  ...
  have h_quad : (∑ h ∈ Finset.Icc 1 N, ∑ k ∈ Finset.Icc 1 N,
    ...
    calc (∑ h ∈ Finset.Icc 1 N, ∑ k ∈ Finset.Icc 1 N,
        cutoffMobiusCoeff N h * cutoffMobiusCoeff N k *
          ((Real.log (2 * Real.pi) - Real.eulerMascheroniConstant) / 2 * (1 / (h : ℝ) + 1 / (k : ℝ)) +
            ((k : ℝ) - (h : ℝ)) / (2 * (h : ℝ) * (k : ℝ)) * Real.log ((h : ℝ) / (k : ℝ)) -
            Real.pi / (2 * (h : ℝ) * (k : ℝ)) *
      ...
      = ∑ h ∈ Finset.Icc 1 N, ∑ k ∈ Finset.Icc 1 N,
        ...
        intro k _
        ring
      _ = (∑ h ∈ Finset.Icc 1 N, ∑ k ∈ Finset.Icc 1 N, cutoffMobiusCoeff N h * cutoffMobiusCoeff N k * ((Real.log (2 * Real.pi) - Real.eulerMascheroniConstant) / 2 * (1 / (h : ℝ) + 1 / (k : ℝ)))) + (∑ h ∈ Finset.Icc 1 N, ∑ k ∈ Finset.Icc 1 N, cutoffMobiusCoeff N h * cutoffMobiusCoeff N k * (((k : ℝ) - (h : ℝ)) / (2 * (h : ℝ) * (k : ℝ)) * Real.log ((h : ℝ) / (k : ℝ)))) + (∑ h ∈ Finset.Icc 1 N, ∑ k ∈ Finset.Icc 1 N, cutoffMobiusCoeff N h * cutoffMobiusCoeff N k * (- (Real.pi / (2 * (h : ℝ) * (k : ℝ)) * (RH.Criteria.NymanBeurling.VasyuninGram.cotangentSumVFormula h k + RH.Criteria.NymanBeurling.VasyuninGram.cotangentSumVFormula k h)))) := by

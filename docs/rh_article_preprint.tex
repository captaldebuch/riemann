...
H15 Phase 7b.3 (Final) & \textbf{100\% COMPLETE} & 0 & 0 & 5e03955 \\
Classical Hyp. 1 & Explicit & — & — & FinalAnalyticAssembly.lean:302 \\
Classical Hyp. 2 & Explicit & — & — & ExactCancellationTarget.lean:90 \\
\bottomrule
\end{tabular}
...
\section{The Two Explicit Classical Hypotheses}
\label{sec:remaining}

The conditional reduction is now complete and precise: exactly two classical analytic theorems are stated as explicit hypotheses at the final assembly endpoint (\CodeName{FinalAnalyticAssembly.lean:302}, commit \commit{5e03955}). These hypotheses are not hidden or assumed implicitly—they are explicit \Lean assumptions in the proof term.

\subsection{Hypothesis 1: Canonical-Product Truncation Bound}

\textbf{Formal statement (Lean, FinalAnalyticAssembly.lean):}
\[
\text{For each height } T, \text{ the finite truncation } P_T(s) = \prod_{|\rho| \le T}(1 - s/\rho)
\]
\[
\text{satisfies } |P_T(s)| \le C(T) \cdot e^{B(T) |s|}
\]
\[
\text{uniformly on the critical strip } \text{Re}(s) \in [0, 1].
\]

\textbf{Mathematical source:} This follows from Hadamard factorization theory for entire functions of genus 1:
\begin{itemize}
  \item The functional equation and critical-line hypothesis (\RH)
  \item Classical bounds on $|\xi(s)|$ on vertical lines
  \item Order-of-growth estimates for entire functions
\end{itemize}

\textbf{Formal location:} \CodeName{XiQuotientGrowth.lean} (line 138 and dependencies). The entire quotient-growth pipeline (commits \commit{bb731ed}--\commit{1bad0e5}) is formalized as infrastructure; this single hypothesis gates the infinite-product characterization.

\textbf{Why Mathlib lacks it:} Mathlib has infinite-product convergence theorems but not finite-truncation bounds that respect zero structure and dynamical growth rates. The truncation requires explicit genus-one machinery not yet in the library.

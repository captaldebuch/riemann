...
\textbf{Status:} Phases 1--7 complete. Phase 7b: algebraic structure and reduction complete; two classical analytic theorems remain.
...
H15 Phase 7b.2 & \textbf{structures wired} & 0 & 0 & 1bad0e5 \\
Classical Input 1 & ⏳ & — & — & (certificate only) \\
Classical Input 2 & ⏳ & — & — & (certificate only) \\
\bottomrule
\end{tabular}
\caption{Formalization status by component. All verified code uses only standard \Lean axioms.}
\label{tab:status}
\end{table}
...
\section{The Two Remaining Classical Theorems}
\label{sec:remaining}

The conditional reduction is now precise: exactly two classical analytic theorems gate \RH completion.

\subsection{Classical Input 1: Canonical-Product Truncation Bound}

\textbf{Formal statement (Lean):}
\[
...
\textbf{Mathematical source:} This is a classical result in Hadamard factorization theory for entire functions of genus 1. It follows from:
\begin{itemize}
...
\textbf{Why Mathlib lacks it:} Mathlib has infinite-product convergence theorems but not finite-truncation bounds that respect zero structure and growth rates. The truncation requires explicit genus-one machinery.

\subsection{Classical Input 2: BCF Integrated Cancellation}

\textbf{Formal statement (Bettin--Conrey--Farmer, Lemmas 2--3):}

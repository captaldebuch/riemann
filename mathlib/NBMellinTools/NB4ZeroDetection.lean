...
This file transports the Field 1 Mellin continuity estimate through finite
Báez--Duarte approximants.  Assuming the finite-approximation form of the
Nyman--Beurling criterion, a zeta zero in `1/2 < Re(s) < 1` would force the
Mellin transform of `χ_(0,1]` to vanish, contradicting its exact value `1/s`.

The result is deliberately one-sided.  No zero symmetry, functional equation,
or Riemann-hypothesis assumption is imported here.
-/

namespace NBMellinTools.NB4
...
/-- An abstract Field 1 estimate transports generator vanishing to the
vanishing of the target transform. -/
theorem vanishing_transport_of_field_1
  ...
  obtain ⟨C, hC, hbound⟩ := hfield1 s₀ hs0 hs1
  by_contra hchi_ne
  ...
  · have hCpos : 0 < C := lt_of_le_of_ne hC (Ne.symm hCzero)
    have htarget :
        0 < (‖mellin (fun x : ℝ => (chi01 x : ℂ)) s₀‖ / C) ^ 2 := by
      positivity
...
/-- Field 2 instantiated with the proved Field 1 continuity estimate. -/
theorem vanishing_transport
    (s₀ : ℂ) (hs0 : 1 / 2 < s₀.re) (hs1 : s₀.re < 1)
    (hcriterion : NymanBeurlingCriterion)
    (hgens : ∀ k : ℕ,
      mellin (fun x : ℝ => (rhoBD k x : ℂ)) s₀ = 0) :
  ...
  vanishing_transport_of_field_1
    NBMellinTools.NB3.mellin_evaluation_continuous_on_critical_strip
    s₀ hs0 hs1 hcriterion hgens

/-- The finite-approximation Nyman--Beurling criterion rules out zeta zeros
in the right half of the critical strip.  The left half is intentionally not
claimed here: completing it requires a separately audited symmetry bridge. -/
theorem riemannZeta_ne_zero_of_nymanBeurlingCriterion
    (hcriterion : NymanBeurlingCriterion) {s : ℂ}
    (hs0 : 1 / 2 < s.re) (hs1 : s.re < 1) :
    riemannZeta s ≠ 0 := by
  intro hzeta
  have hgens : ∀ k : ℕ,
      mellin (fun x : ℝ => (rhoBD k x : ℂ)) s = 0 := by
    intro k
    rw [mellin_rhoBD k (by linarith [hs0]) hs1, hzeta]
    simp
  have hchi := vanishing_transport s hs0 hs1 hcriterion hgens
  have hspos : 0 < s.re := by linarith [hs0]
  rw [mellin_chi01 hspos] at hchi
  exact (one_div_ne_zero (ne_zero_of_re_pos hspos)) hchi

end

end NBMellinTools.NB4

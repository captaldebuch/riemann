...
import RiemannHypothesis.Criteria.NymanBeurling.Generated.VasyuninIntervalsN1
import Mathlib
...
namespace RH.Certificates.Generated.NymanGramN1
...
def entry_1_1 : GramEntryIntervalCertificate :=
{
  ...
  certified_interval := RH.Criteria.NymanBeurling.Generated.VasyuninIntervalsN1.interval_1_1.interval_proof
}


def M_reg_fn (i j : Fin 1) : ℚ :=
  if i.val == 0 ∧ j.val == 0 then (113228981/89810000)
  else 0

def L_val_fn (i j : Fin 1) : ℚ :=
  if i.val == 0 ∧ j.val == 0 then 1
  else 0

def D_val_fn (i : Fin 1) : ℚ :=
  if i.val == 0 then (113228981/89810000)
  else 0

def linear_fn (i : Fin 1) : ℚ :=
  if i.val == 0 then (334885130399/792094461875)
  else 0

def coeff_fn (i : Fin 1) : ℚ :=
  if i.val == 0 then (135996543/405547645)
  else 0

def M_reg : Matrix (Fin 1) (Fin 1) ℚ := M_reg_fn
def L_val : Matrix (Fin 1) (Fin 1) ℚ := L_val_fn
def D_val : Fin 1 → ℚ := D_val_fn

-- Provide interval models
def gram_lower (i j : Fin 1) : ℚ :=
  if i.val == 0 ∧ j.val == 0 then entry_1_1.lower
  else 0

def gram_upper (i j : Fin 1) : ℚ :=
  if i.val == 0 ∧ j.val == 0 then entry_1_1.upper
  else 0

-- Legacy small-N interval data.
def lin_lower (i : Fin 1) : ℚ := linear_fn i - (1/1000000000)
def lin_upper (i : Fin 1) : ℚ := linear_fn i + (1/1000000000)

axiom lin_certified : ∀ k : Fin 1,
...
theorem toy_gram_certified (i j : Fin 1) :
...
def Q_interval : BaezDuarteIntervalQuadraticModel 1 :=
{
...
def witnessN1 : RationalPSDWitness 1 :=
{
  M := M_reg
  L := L_val
  D := D_val
...
def regCertN1 : RegularizedPSDCertificate 1 :=
{
...
def witnessEnergy : FiniteIntervalEnergyWitness 1 :=
{
  model := Q_interval
  coeffs := coeff_fn
  energyUpper := (17888648608580391148526475607463333/20843983806890948357117247500000000)
  interval_energy_bound := True.intro
}


noncomputable def certN1 : FiniteApproximationCertificate :=
{
  N := 1
...
theorem N1_quadratic_le_energyUpper :
    ∑ h : Fin 1, ∑ k : Fin 1,
  ...
  have hbound := sign_aware_energy_bound (N := 1) coeff_fn
  ...
  have heq : signAwareQuadBoundQ coeff_fn gram_lower gram_upper +
    ...
    norm_num [signAwareQuadBoundQ, signAwareLinBoundQ, coeff_fn,
      ...
      entry_1_1, witnessEnergy]
  calc ∑ h : Fin 1, ∑ k : Fin 1,
          (coeff_fn h : ℝ) * (coeff_fn k : ℝ) *
          RH.Criteria.NymanBeurling.VasyuninGram.baezDuarteGramEntry (h.val + 1) (k.val + 1)
        - 2 * ∑ k : Fin 1, (coeff_fn k : ℝ) * innerProductChiRho k.val + 1
...
theorem baezDuarteDistance_N1_bound :
    RH.Criteria.NymanBeurling.BaezDuarte.BaezDuarteDistance 1 ≤
  ...
  exact N1_quadratic_le_energyUpper

end RH.Certificates.Generated.NymanGramN1
